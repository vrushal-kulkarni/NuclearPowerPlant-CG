...
int enter;
float i,j;
float x2,y2,r,r1=80,r2=5.0,r3=27.5,r4=6,r5=6,x,y,angle,angle_radians,r6=23,r7=2,r8=3,r9=6,r10=18,r11=10,r12=26,r13=14,R=23,Xc=255,Yc=138,X1=48,X2=48,Y1=122,Y2=150;
int k=0;
int p=0;
...
glBegin(GL_POINTS);		//Specifies the primitives that will be created	from vertices presented	between	glBegin	and the subsequent glEnd
glVertex2f(255,138);
glEnd();
...
void rot1()					//dis is for rotating turbine shaft(logic is to create 12 triangles,3 each from fig1,2,3,4 by calling them and flushing)
{
glDisable(GL_DEPTH_TEST);     // the depth buffer is not updated if the depth test is disabled.....enable or disable server-side GL capabilities, Most state variables can be enabled or disabled with glEnable() or glDisable()
glColor3f(0.0,0.0,0.0);
	fig1();
	glFlush();					//glFlush() guarantees that all OpenGL commands made up to that point will complete executions in a finite amount time after calling glFlush()
...
void Circle8Points(int a, int b) 
 { 
...
void CircleMidpoint(void) 
 { 
           int a=0; 
           int b=R; 
           int c=1-R;       //initial value of the decision parameter 
 
		   glPointSize(6);
           glBegin(GL_POINTS); 
				glColor3f(0.3,0.1,0.1);
                glVertex2f(Xc,Yc);            //Draw circle center for illustration 
                Circle8Points(a,b);/* Plot the First point */ 
 while(a<b) 
                { 
                      a++; 
                      if(c<0) 
...
void turbine()					//dis is for darwing 2 circles,,here also circle drawing algo can b used 
{								//centre=255,138....r(inner)=23,r(outer)=26
	glLineWidth(6.0);
	CircleMidpoint();
			glLineWidth(6.0);
...
setPixel(round (x), round(y));// Computer Graphics with OpenGL
for(k=0; k<steps; k++)
{
x += xIncrement;
y += yIncrement;
...
void drawMyLine(GLint l, GLint m, GLint n, GLint o)
{
...
glColor3f(0.6,0.6,0.4);
drawMyLine(50,122,50,185);
LineBres();		//problem with bresenhams line drawing algo is shown i.e. it cant be used for drawing vertical lines as slope!<1
drawMyLine(52,122,52,185);	
delay(500);
...
glPushMatrix();		// after a glPushMatrix call, the matrix on top of the stack is identical to the one below it
while(p<200)
{
flag=1;
if(flag==1)
...
glPopMatrix();		//glPopMatrix pops the current matrix stack, replacing the current matrix with the one below it on the stack...There is a stack of matrices for each of the matrix modes
points();
conecting_pipe();
 ...
 void drawstring(float x,float y,float z,char *string)
{
	...
	{	glColor3f(0.0,1.0,1.0);
		glutBitmapCharacter(currentfont,*c);	//A bitmap font is basically a 2D font..functions to put some bitmapped text on the screen
	}
}
...
void init(void)
{
  ...
  glEnable(GL_DEPTH_TEST);
	gluOrtho2D(0.0,400.0,0.0,300.0);		//gluOrtho2D sets up a two-dimensional orthographic viewing region
...
glEnable(GL_BLEND);				//Blending is commonly used to make objects translucent
   ...
   glutAttachMenu(GLUT_RIGHT_BUTTON);
	glutDisplayFunc(disp);   //contents of the window need to be redisplayed
